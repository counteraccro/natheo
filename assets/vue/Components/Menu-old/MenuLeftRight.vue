      ...
      menuElement.children.forEach((element) => {
        ...
        if (!element.disabled) {
          if (!this.isHaveChildren(element)) {
            html +=
              '<li><a class="link-body-emphasis d-inline-flex text-decoration-none rounded element-hover"  target="' +
              element.linkTarget +
              '" href="' +
            ...
            html +=
              '<li><a href="#" class="link-body-emphasis d-inline-flex text-decoration-none rounded link-toggle element-hover no-control" data-bs-toggle="collapse" data-bs-target="#demo-' +
              element.id +
              '" aria-expanded="false"> ' +
              elementTranslate.text +
              '</a>';
...
<template>
  <div
    class="flex-shrink-0 p-3 bg-body-tertiary border border-1 border-light"
    :class="this.definePosition()"
    style="width: 280px"
  >
    <a
    ...
    <ul v-if="this.type === 11" class="list-unstyled ps-0">
      ...
      >
        <a
          ...
          class="btn d-inline-flex align-items-center rounded border-0 collapsed element-hover"
    ...
    <ul v-if="this.type === 12" class="list-unstyled ps-0">
      ...
      >
        <a
          v-if="!this.isHaveChildren(element) && !element.disabled"
          class="btn d-inline-flex align-items-center rounded border-0 collapsed element-hover"
          :target="element.linkTarget"
          :href="elementTranslate.link"
          >{{ elementTranslate.text }}
