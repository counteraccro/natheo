  ...
  <nav>
      ...
      <div class="relative flex items-center justify-between h-16">
            ...
            <div class="flex space-x-4 items-center h-16">
              ...
              <div v-for="element in this.data.elements" v-if="this.data.type === this.MenuType.headerDropDown">
                <a
                  ...
                  >{{ element.label }}</a
                >
                <NavMenuDropdown v-else :utils-front="this.utilsFront" :data="element" />
              </div>

              <div
                v-for="element in this.data.elements"
                v-if="
                  this.data.type === this.MenuType.headerBigMenu ||
                  this.data.type === this.MenuType.headerBigMenu2column ||
                  this.data.type === this.MenuType.headerBigMenu3column ||
              ...
              >
                ...
                <NavMegaMenu v-else :utilsFront="this.utilsFront" :data="element" :type="this.data.type" />
              </div>
            </div>
          </div>
        </div>
        <div class="absolute inset-y-0 right-0 flex items-center pr-2 lg:static lg:inset-auto lg:ml-6 lg:pr-0">
          <div class="hidden lg:flex lg:items-center">
            ...
            >
              <svg
                class="fill-violet-700 block group-hover:fill-theme-1-100 dark:hidden"
                fill="currentColor"
                viewBox="0 0 20 20"
              >
            ...
            <a
              :href="this.urls.adminAuth"
              v-if="Object.keys(this.userInfo).length === 0"
            ...
            <div v-else class="flex items-center space-x-2 focus:outline-none">
              <div class="relative">
                <div
                  v-if="this.userInfo.avatarImg === null || this.userInfo.avatarImg === ''"
                ...
                <div
                  v-else
                  class="relative inline-flex items-center justify-center w-10 h-10 overflow-hidden rounded-full mt-1"
                >
              ...
              <i
                class="fas fa-chevron-down text-xs text-gray-500 hidden lg:inline transition-transform duration-200 group-hover:text-blue-600"
              ></i>
            </div>
          </div>
    ...
    <div class="lg:hidden hidden" id="mobile-menu">
      <div class="px-2 pt-2 pb-3 space-y-1">
        <a
          v-for="element in this.data.elements"
          v-if="this.data.type === this.MenuType.headerSideBar"
          :href="this.utilsFront.getUrl(element)"
        ...
        <ul v-if="this.data.type !== this.MenuType.headerSideBar">
          <nav-menu-dropdown-mobile :data="this.data" :utilsFront="this.utilsFront" />
        </ul>
        <div class="pt-4 pb-3 border-t border-gray-200">
          <div class="flex items-center px-3 space-y-2 flex-col">
            <div
              class="group block w-full text-center hover:bg-theme-4-750 hover:!text-theme-1-100 rounded-md text-base cursor-pointer hover:dark:bg-gray-600"
              @click="this.switchDarkMode()"
            >
              <div class="h-10 w-10 rounded-lg p-2 m-auto">
                <svg
                  class="fill-violet-700 group-hover:fill-theme-1-100 block dark:hidden"
                  fill="currentColor"
                  viewBox="0 0 20 20"
                >
                  <path d="M17.293 13.293A8 8 0 016.707 2.707a8.001 8.001 0 1010.586 10.586z"></path>
