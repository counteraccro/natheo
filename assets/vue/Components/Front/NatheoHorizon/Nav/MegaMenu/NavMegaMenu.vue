  ...
  methods: {
    ...
    constructArray(data) {
      for (let key in data.elements) {
        ...
        } else this.elements.push(data.elements[key]);
      }
    },
    ...
    getNbCol(data) {
      ...
      for (let key in data.elements) {
        if (data.elements[key].hasOwnProperty('elements')) {
          nb++;
        } else if (noChildren < 1) noChildren++;
      }
...
<template>
  <div class="relative group inline-block">
    ...
    <!-- Mega menu panel -->
    <div
      class="absolute left-0 hidden group-hover:block group-focus-within:block bg-white border border-gray-200 rounded-lg shadow-lg z-20 w-[700px] opacity-0 translate-y-3 transition-all duration-300 ease-out group-hover:opacity-100 group-hover:translate-y-0 group-focus-within:opacity-100 group-focus-within:translate-y-0"
    >
      <div class="grid gap-6 p-6" :class="'grid-cols-' + this.getNbCol(this.data)">
        ...
        <div v-for="element in this.elementsChild">
          ...
          <nav-mega-menu-sub :data="element.elements" :utils-front="this.utilsFront" :deep="0" />
        </div>
