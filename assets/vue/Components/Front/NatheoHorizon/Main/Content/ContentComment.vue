...
<template>
  ...
  <div v-if="this.isLoad" class="mx-auto max-w-4xl p-4 sm:p-6" id="ancre-comment">
    <!-- Header -->
    <div
      class="flex flex-wrap items-center justify-between rounded-2xl border border-neutral-200/70 bg-white p-4 shadow-sm"
    >
      ...
      <div>
        <button
          type="button"
          @click="this.renderBlock('block-add-comment', 'show')"
      ...
      <span class="text-sm text-slate-600 mt-3 md:mt-0">{{ this.nbElements }} {{ this.translate.nbComments }}</span>
    </div>

    <div
      id="block-add-comment"
      class="mx-auto max-w-4xl bg-white rounded-2xl border border-neutral-200/70 p-4 shadow-sm space-y-4 mt-5 opacity-0 animate-fadeInUp hidden"
    >
      <div>
        <label for="pseudo" class="block text-sm font-medium text-gray-700 mb-1"
          >{{ this.translate.formNewCommentPseudoLabel }} *</label
        >
        <input
          type="text"
          id="pseudo"
          name="pseudo"
          v-model="this.newComment.author"
          ...
          :class="!this.validateNewComment.author ? 'invalid:border-red-500 invalid:text-red-600' : ''"
          required
        />
      ...
      <div>
        <label for="email" class="block text-sm font-medium text-gray-700 mb-1">{{
          this.translate.formNewCommentEmailLabel
        }}</label>
        <input
          type="email"
          id="email"
      ...
      <div>
        <label for="commentaire" class="block text-sm font-medium text-gray-700 mb-1"
          >{{ this.translate.formNewCommentCommentLabel }} *</label
        >
        <textarea
          id="commentaire"
          name="commentaire"
          rows="4"
          :placeholder="this.translate.formNewCommentCommentPlaceholder"
          v-model="this.newComment.comment"
          class="w-full rounded-xl border border-gray-300 px-3 py-2 focus:border-theme-4-750 focus:ring focus:ring-blue-200 outline-none"
          :class="!this.validateNewComment.comment ? 'invalid:border-red-500 invalid:text-red-600' : ''"
          required
        ></textarea>
      ...
      <div class="flex justify-end space-x-3">
        <button
          type="button"
          @click="this.renderBlock('block-add-comment', 'remove')"
    ...
    <div class="mt-6 space-y-4">
      <div
        ...
        class="rounded-2xl border border-neutral-200/70 p-4 shadow-sm"
        :class="this.colorComment(comment.status)"
      >
        <div class="flex gap-3">
          ...
          <div>
            <div class="flex items-center gap-2">
              ...
              <span class="text-xs text-slate-600">• {{ this.utilsFront.timeAgo(comment.createdAt * 1000) }}</span>
            </div>

            <div
              class="mt-2 text-sm leading-relaxed text-slate-600 natheo-content-text"
              v-html="this.output(comment.comment)"
            ></div>
            <div
              v-if="comment.moderate"
              class="mt-2 text-sm leading-relaxed text-slate-800 italic"
        ...
        <div class="text-[0.7em] text-right" v-if="this.utilsFront.isUserCanModerate() && this.isLoadModerate">
          <a
            ...
            @click="this.moderateComment(comment.id, CommentStatus.validate)"
            class="hover:bg-green-600 hover:!text-theme-1-100 rounded-md hover:dark:bg-gray-600 p-1"
            v-if="comment.status !== CommentStatus.validate"
            >{{ this.translate.validate }}</a
          >
          <a
            :href="'#comment-' + comment.id"
            @click="this.renderBlock('block-moderate-comment-' + comment.id, 'show')"
            class="hover:bg-red-600 hover:!text-theme-1-100 rounded-md hover:dark:bg-gray-600 p-1"
            v-if="comment.status !== CommentStatus.moderate"
            >{{ this.translate.moderate }}</a
          >
          <a
            :href="'#comment-' + comment.id"
            @click="this.moderateComment(comment.id, CommentStatus.waitValidation)"
            class="hover:bg-orange-300 hover:!text-theme-1-100 rounded-md hover:dark:bg-gray-600 p-1"
            v-if="comment.status !== CommentStatus.waitValidation"
            >{{ this.translate.waiting }}</a
          >
        </div>
        ...
        <div :id="'block-moderate-comment-' + comment.id" class="opacity-0 translate-y-4 animate-fadeInUp mb-5 hidden">
          ...
          <textarea
            ...
            rows="4"
            class="w-full rounded-xl border border-gray-300 px-3 py-2 focus:border-theme-4-750 focus:ring focus:ring-blue-200 outline-none"
            :placeholder="this.translate.formModeratePlaceHolder"
            v-model="this.textModerateComment"
          ></textarea>
          ...
          <div class="flex justify-end space-x-3 mt-4">
            <button
              type="button"
              @click="this.renderBlock('block-moderate-comment-' + comment.id, 'remove')"
              class="bg-gray-200 text-gray-700 font-medium py-2 px-4 rounded-xl hover:bg-gray-300 transition cursor-pointer"
            >
            ...
            <button
              type="submit"
              @click="this.moderateComment(comment.id, CommentStatus.moderate)"
              class="text-slate-600 font-medium py-2 px-4 rounded-xl hover:bg-theme-4-750 transition border-gray-200 border-1 hover:border-theme-4-750 hover:text-white cursor-pointer"
            >
      ...
      <div v-if="this.nbElements > 0">
        <nav
          class="content-paginate flex flex-wrap gap-y-3 items-center justify-center mt-8 space-x-2"
        ...
        >
          ...
          <div v-for="(n, i) in this.getNbPage()" :id="'p-comment-' + n">
            <a
              ...
              v-if="
                n === this.page - 1 || n === this.page + 1 || n === this.page || n <= 2 || n >= this.getNbPage() - 1
              "
              >{{ n }}</a
            >
            <a
