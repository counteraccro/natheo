...
<template>
  ...
  <div class="modal-body">
    <div class="row">
      <div class="col-8">
        <div v-if="!IsEmpty(this.data)" v-for="(data, label) in this.data.data">{{ label }} : {{ data }}</div>
      </div>
      <div class="col-4 d-flex justify-content-center align-items-center text-center">
        ...
        <div v-else-if="!IsEmpty(this.data) && this.data.type === 'media'">
          ...
          <a class="link-dark" :href="this.data.web_path" target="_blank">
            <span v-if="this.data.media_type === 'img'"
              ><i class="bi bi-arrows-angle-expand"></i> {{ this.translate.link_size }}
            </span>
