...
<template>
  <div class="modal-header bg-secondary">
    <h1 class="modal-title fs-5 text-white"><i class="bi bi-info-circle-fill"></i> {{ this.translate.title }}</h1>
  ...
  <div class="modal-body">
    <div class="card border border-secondary" :class="this.loading === true ? 'block-grid' : ''">
      ...
      <div class="card-header text-bg-secondary">
        ...
        <MediasBreadcrumb :paths="this.currentFolder.root" @load-folder="this.loadMediaInFolder"> </MediasBreadcrumb>
      </div>
      <div class="card-body">
        <div class="dropdown float-end">
          ...
          <ul class="dropdown-menu">
            <li>
              <a class="dropdown-item no-control" style="cursor: pointer" @click="this.setSize('fluid')"
                ><i class="bi bi-aspect-ratio"></i> {{ this.translate.size_fluide }}</a
              >
            </li>
            <li>
              <a class="dropdown-item no-control" style="cursor: pointer" @click="this.setSize('max')"
                ><i class="bi bi-card-image"></i> {{ this.translate.size_max }}</a
              >
            </li>
            <li>
              <a class="dropdown-item no-control" style="cursor: pointer" @click="this.setSize('100')"
                ><i class="bi bi-textarea-resize"></i> {{ this.translate.size_100 }}</a
              >
            </li>
            <li>
              <a class="dropdown-item no-control" style="cursor: pointer" @click="this.setSize('200')"
                ><i class="bi bi-textarea-resize"></i> {{ this.translate.size_200 }}</a
              >
            </li>
            <li>
              <a class="dropdown-item no-control" style="cursor: pointer" @click="this.setSize('300')"
                ><i class="bi bi-textarea-resize"></i> {{ this.translate.size_300 }}</a
              >
            </li>
            <li>
              <a class="dropdown-item no-control" style="cursor: pointer" @click="this.setSize('400')"
                ><i class="bi bi-textarea-resize"></i> {{ this.translate.size_400 }}</a
              >
            </li>
            <li>
              <a class="dropdown-item no-control" style="cursor: pointer" @click="this.setSize('500')"
                ><i class="bi bi-textarea-resize"></i> {{ this.translate.size_500 }}</a
              >
            </li>
          </ul>
        </div>
        <div id="block-media-grid-markdown" class="mt-5 row">
          <div v-if="this.medias.length > 0" class="media col-auto mb-4" v-for="media in this.medias">
            <img
              ...
              :alt="media.name"
              @click="$emit('select-media', media.name, media.webPath, this.size)"
            />
            <div
              v-else
              class="folder"
              alt="media.name"
              @click="$emit('load-media', media.id, 'asc', 'created_at')"
            ></div>
