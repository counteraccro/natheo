  ...
  <table class="table table-striped table-hover" v-if="filteredData.length">
    ...
    <tr v-for="entry in filteredData">
      <td v-for="key in columns">
        ...
        <div v-else>
          <button class="btn btn-secondary btn-sm m-1" v-for="data in this.jsonParse(entry[key])" @click="$emit('redirect-action', data.url, data.ajax, data.confirm)" v-html="data.label">
          </button>
        </div>
      </td>
    </tr>
