  ...
  methods: {
    ...
    delete(confirm) {
      ...
      if (!confirm) {
        ...
        axios.post(this.url_delete, {}).then((response) => {
    ...
    disabled(confirm) {
      ...
      if (!confirm) {
        ...
        axios.post(this.url_disabled, {}).then((response) => {
          this.msg_return = response.data.msg;
          this.url_reload = response.data.redirect;
        }).catch((error) => {
          console.log(error);
...
<template>
  ...
  <!-- Modal disabled -->
  <div class="modal fade" id="modal-alert-disabled" tabindex="-1" aria-labelledby="modal-alert" aria-hidden="true"
       data-bs-backdrop="static" data-bs-keyboard="false">
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        <div class="modal-header bg-danger">
          <h1 class="modal-title fs-5 text-white">{{ this.translate.disabled_title }}</h1>
          <button v-if="!this.confirm_disabled" type="button" class="btn-close" data-bs-dismiss="modal"
  ...
  <!-- Modal delete -->
  <div class="modal fade" id="modal-alert-delete" tabindex="-1" aria-labelledby="modal-alert" aria-hidden="true"
       data-bs-backdrop="static" data-bs-keyboard="false">
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        <div class="modal-header bg-danger">
          ...
          <button v-if="!this.confirm_delete" type="button" class="btn-close" data-bs-dismiss="modal"
                  aria-label="Close"></button>
        </div>
        <div class="modal-body">
        ...
        <div class="modal-footer">
          <button v-if="!this.confirm_delete && this.isDelete()" type="button" class="btn btn-secondary"
                  data-bs-dismiss="modal">{{ this.translate.delete_btn_cancel }}
          </button>
          <button v-if="!this.confirm_delete && this.isReplace()" type="button" class="btn btn-secondary"
                  data-bs-dismiss="modal">{{ this.translate.replace_btn_cancel }}
          </button>
