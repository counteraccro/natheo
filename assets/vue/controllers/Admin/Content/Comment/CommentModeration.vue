...
export default {
  ...
  methods: {
    ...
    moderateComment() {
      this.loading = true;
      axios
        .post(this.urls.update, this.moderation)
        .then((response) => {
...
<template>
  <div id="block-moderation" :class="this.loading === true ? 'block-grid' : ''">
      ...
      <div class="row">
        <div class="col-4">
          ...
          <select class="form-select" v-model="this.filters.status" id="list-status" @change="this.load()">
            ...
            <option
              ...
              :value="status"
              :selected="status === this.filters.status"
            >
              {{ key }}
            </option>
          </select>
        ...
        <div class="col-4">
          ...
          <select class="form-select" v-model="this.filters.pages" id="list-pages" @change="this.load()">
            ...
            <option v-for="(key, status) in this.datas.pages" :value="status" :selected="status === this.filters.pages">
              {{ key }}
            </option>
          </select>
        </div>
        <div class="col">
          <div class="btn btn-secondary float-end mt-3 me-3" @click="this.resetSelected()">
    ...
    <fieldset class="mb-3" v-if="this.moderation.selected.length > 0">
      ...
      {{ this.moderation.selected.length }} {{ this.translate.selection_comment }} :<span
        v-for="id in this.moderation.selected"
      >
        #{{ id }},</span
      >
        ...
        <div class="col">
          <div v-if="this.moderation.status === '3'">
            ...
            <textarea
              ...
              rows="2"
              v-model="this.moderation.moderateComment"
            ></textarea>
          </div>
        </div>
      </div>
      <div class="btn btn-secondary float-end" @click="this.moderateComment()">
        {{ this.translate.selection_submit }}
      </div>
    </fieldset>
        ...
        <div class="card mb-3" :class="this.isChecked(comment.id) ? 'border-2 border-secondary' : ''">
          <div class="card-header">
            <input
              ...
              :checked="this.isChecked(comment.id)"
            />
