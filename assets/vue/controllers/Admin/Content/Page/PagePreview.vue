  ...
  methods: {
    ...
    async loadBlockContent() {
      for (const element of this.page.contents) {
        ...
        await axios
          .get(this.urls.apiGetContent + '?id=' + element.id + '&locale=' + this.datas.locale, {
            headers: {
              Authorization: 'Bearer ' + this.datas.token,
            },
          })
    ...
    renderContent(position) {
      return this.page.contents.filter((content) => content.position === position);
    },
...
<template>
  <div id="global-menu" :class="this.loading === true ? 'block-grid' : ''">
    ...
    <header>
      <preview-header v-if="this.header" :p-menu="this.header" :data="this.datas.site"> </preview-header>
    </header>
    <div class="row">
      <nav v-if="this.menuLeft" class="col-2">
        <PreviewMenuLeftRight :p-menu="this.menuLeft" :data="this.datas.site"></PreviewMenuLeftRight>
      </nav>
      ...
      <nav v-if="this.menuRight" class="col-2">
        <PreviewMenuLeftRight :p-menu="this.menuRight" :data="this.datas.site"></PreviewMenuLeftRight>
      </nav>
    </div>
    <PreviewFooter v-if="this.footer" :p-menu="this.footer" :data="this.datas.site"></PreviewFooter>
    <div v-else></div>
  </div>
</template>
